...
<div x-show="activeTab === 'slips'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6" x-cloak>
    ...
    @else
                        ...
                        <template x-for="item in paginatedSlips()" :key="item.employee_id">
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    <span class="text-[10px] text-gray-400 font-semibold block mt-0.5" x-text="item.days_worked + ' / ' + item.total_days + ' HK'"></span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button @click="showSlipModal = true; selectedSlip = { 
                                                    ...
                                                    gaji_full: item.gaji_full,
                                                    tarif_harian: item.tarif_harian,
                                                    pokok: item.pokok, 
                                                    lembur: item.lembur, 
                                                    risiko: item.risiko, 
                                                    lain_lain: item.lain_lain,
                                                    bpjs_health: item.bpjs_health,
                                                    bpjs_tk: item.bpjs_tk,
                                                    pph21: item.pph21,
                                                    potongan_absen: item.potongan_absen,
                                                    potongan: item.potongan,
                                                    total: new Intl.NumberFormat('id-ID').format(item.total),
                                                    type: 'pkwt',
                                        ...
                                        <form :action="'/payroll/pkwt/periods/{{ $period->id }}/slip/send/' + item.employee_id" 
                                              method="POST" 
                                              class="inline"
        ...
        <div x-show="slipsList.length > 0" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-white/[0.03] rounded-b-2xl">
            <div class="flex flex-wrap items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <x-form.select-pagination model="slipsPerPage" page="slipsPage" />
                </div>
            </div>
