    ...
    <div x-show="showModal" 
        ...
        <div @click.away="showModal = false" 
            ...
            <div class="mb-8 pr-10">
                ...
                <p class="mt-1.5 text-sm text-gray-500 dark:text-gray-400">Daftarkan pengguna baru untuk memberikan akses ke sistem penggajian.</p>
            </div>
            ...
            <form class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <!-- Nomor HP -->
                    <x-form.input label="Nomor HP" name="phone" placeholder="Contoh: 081234567890" required />
                ...
                <div class="flex items-center justify-between rounded-2xl border border-gray-100 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-white/[0.02]">
                    ...
                    <div x-data="{ active: true }" @click="active = !active" 
                         class="relative cursor-pointer rounded-full transition-all duration-200 focus:outline-none" 
                         :class="active ? '' : 'bg-gray-200 dark:bg-gray-700'"
                         :style="active ? 'width: 44px; height: 24px; background-color: #10b981;' : 'width: 44px; height: 24px;'">
                        <div class="absolute rounded-full bg-white shadow-sm transition-all duration-200" 
                             :style="active ? 'top: 2px; left: 22px; width: 20px; height: 20px;' : 'top: 2px; left: 2px; width: 20px; height: 20px;'"></div>
                ...
                <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-800">
                    <x-ui.button variant="outline" type="button" @click="showModal = false" className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">Batal</x-ui.button>
                    <x-ui.button variant="primary" type="submit" className="shadow-sm shadow-brand-500/10 transition-all hover:translate-y-[-1px]">Simpan User</x-ui.button>
                </div>
            </form>
        </div>
    </div>
