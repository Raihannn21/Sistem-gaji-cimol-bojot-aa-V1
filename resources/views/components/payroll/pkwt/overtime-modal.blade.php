...
<template x-teleport="body">
    <div x-show="showOvertimeModal" x-transition:enter="transition ease-out duration-300"
        ...
        <div @click.away="showOvertimeModal = false" x-show="showOvertimeModal"
            ...
            <h3 class="text-xl font-bold text-gray-800 dark:text-white/90">Input Lembur Karyawan (PKWT)</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tambahkan data lembur bulanan untuk perhitungan gaji karyawan kontrak.</p>

            <form action="{{ $period ? url('/payroll/pkwt/periods/' . $period->id . '/overtime') : '#' }}" method="POST" class="mt-8 space-y-5 pb-40">
                @csrf
                <div class="space-y-5">
