<template x-teleport="body">
    <div x-show="showModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div @click.away="showModal = false" x-show="showModal" x-transition:enter="transition ease-out duration-300"
            ...
            <h3 class="text-xl font-bold text-gray-800 dark:text-white/90">Buka Periode Gaji Baru (PKWT)</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tentukan rentang tanggal untuk mengaktifkan periode penggajian Karyawan Kontrak (PKWT).</p>

            <form action="{{ url('/payroll/pkwt/periods') }}" method="POST" class="mt-8 space-y-5 pb-40">
                @csrf
                <!-- Judul Periode -->
                <x-form.input label="Judul Periode" name="title" placeholder="Contoh: Gaji PKWT Juli 2025" required />
                ...
                <div
                    ...
                    <div class="flex gap-3">
                        <svg class="h-5 w-5 text-brand-500 shrink-0" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-3 pt-4">
                    ...
                    <x-ui.button variant="primary" className="flex-1" type="submit">Buka Periode</x-ui.button>
                </div>
            </form>
        </div>
    </div>
