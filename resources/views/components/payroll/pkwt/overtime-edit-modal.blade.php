...
<template x-teleport="body">
    <div x-show="showEditOvertimeModal"
        ...
        <div @click.away="showEditOvertimeModal = false" x-show="showEditOvertimeModal"
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                <span class="font-semibold text-brand-600" x-text="selectedEmployee.name"></span> | <span
                    x-text="selectedOvertimeDateFormatted"></span>
            </p>

            <form :action="`/payroll/pkwt/periods/{{ $period->id }}/overtime/${selectedOvertimeId}`" method="POST"
                ...
                <div>
                    ...
                    <input type="number" name="hours" x-model="selectedOvertimeHours" required min="1"
                ...
                <x-form.input name="amount" label="Nominal Lembur" prefix="Rp" data-currency placeholder="0" required
                    x-model="selectedOvertimeAmount" @input="formatCurrency($event.target)" />

                <div>
                    <label class="mb-2.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                        Keterangan / Aktivitas
                    </label>
                    <textarea name="note" x-model="selectedOvertimeNote" rows="3"
                        class="w-full rounded-xl border border-gray-200 bg-transparent py-3 px-5 text-gray-800 outline-none focus:border-brand-500 dark:border-gray-800 dark:text-white dark:focus:border-brand-500"
                        placeholder="Jelaskan aktivitas lembur..."></textarea>
                </div>

                <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-800">
                    <x-ui.button variant="outline" type="button"
                        @click="showEditOvertimeModal = false">Batal</x-ui.button>
