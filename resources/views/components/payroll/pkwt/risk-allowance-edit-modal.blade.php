...
<template x-teleport="body">
    <div x-show="showEditRiskModal" 
        ...
        <div @click.away="showEditRiskModal = false" x-show="showEditRiskModal"
            ...
            <h3 class="text-xl font-bold text-gray-800 dark:text-white/90">Edit Tunjangan Risiko (PKWT)</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                <span class="font-semibold text-brand-600" x-text="selectedEmployee.name"></span> | <span x-text="selectedRiskDate"></span>
            </p>

            <form :action="`/payroll/pkwt/periods/{{ $period->id }}/risk/${selectedRiskId}`" method="POST" class="mt-8 space-y-5">
                @csrf
                @method('PUT')
