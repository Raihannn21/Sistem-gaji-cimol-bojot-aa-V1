...
<template x-teleport="body">
    <div x-show="showOthersDetailModal" x-transition:enter="transition ease-out duration-300"
        ...
        class="fixed inset-0 z-999999 flex items-center justify-center bg-gray-400/50 backdrop-blur-sm p-4" x-cloak>
         ...
         <div @click.away="showOthersDetailModal = false" x-show="showOthersDetailModal"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
            x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-200"
            x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
            class="relative w-[600px] max-w-full rounded-3xl bg-white p-6 shadow-xl dark:bg-gray-900 sm:p-8">
              ...
              <div class="flex items-center gap-4 border-b border-gray-100 pb-6 dark:border-gray-800">
                <div
                    class="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-500/10 text-brand-500 font-bold text-xl">
                ...
                <div class="max-h-[300px] overflow-y-auto rounded-2xl border border-gray-100 dark:border-gray-800">
                    <table class="w-full text-left">
                        <thead class="sticky top-0 bg-gray-50 dark:bg-white/[0.03]">
                            <tr>
                                <th class="px-4 py-3 text-[10px] font-black uppercase tracking-wider text-gray-500">Jenis Tunjangan</th>
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                            <template x-for="item in selectedEmployeeOthers" :key="item.id">
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                                    ...
                                    <td class="px-4 py-3 text-sm text-right font-bold text-gray-800 dark:text-white" x-text="'Rp ' + Number(item.amount).toLocaleString('id-ID')"></td>
              ...
              <div class="mt-8 flex justify-end pt-6 border-t border-gray-100 dark:border-gray-800">
                <x-ui.button variant="outline" @click="showOthersDetailModal = false">Tutup</x-ui.button>
            </div>
        </div>
    </div>
</template>
