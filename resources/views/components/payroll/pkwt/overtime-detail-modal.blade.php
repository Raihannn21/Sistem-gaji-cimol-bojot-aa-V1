...
<template x-teleport="body">
    <div x-show="showDetailModal" 
        ...
        <div @click.away="showDetailModal = false" 
            ...
            <button @click="showDetailModal = false" class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 sm:right-6 sm:top-6 transition-colors duration-150">
                ...
                <div class="max-h-[300px] overflow-y-auto rounded-2xl border border-gray-100 dark:border-gray-800 custom-scrollbar">
                    <table class="w-full text-left border-collapse">
                        <thead class="sticky top-0 bg-gray-50 dark:bg-white/[0.03] z-10">
                            <tr>
                                <th class="px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-gray-500">Tanggal</th>
                                <th class="px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-gray-500 text-center">Jam</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            <template x-for="item in selectedEmployeeOvertimes" :key="item.id">
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                                    <td class="px-5 py-3.5 text-sm text-gray-700 dark:text-gray-400" x-text="item.date"></td>
                                    <td class="px-5 py-3.5 text-sm text-center font-bold text-gray-800 dark:text-white" x-text="item.hours + ' Jam'"></td>
                                    ...
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <button @click="$dispatch('open-delete-modal', { 
                                                            url: '/payroll/pkwt/periods/{{ $period->id }}/overtime/' + item.id,
            ...
            <div class="mt-8 flex justify-end pt-6 border-t border-gray-100 dark:border-gray-800">
                <x-ui.button variant="outline" @click="showDetailModal = false" className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">Tutup</x-ui.button>
            </div>
        </div>
    </div>
</template>
