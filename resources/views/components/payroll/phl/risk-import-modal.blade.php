...
<template x-teleport="body">
    <div x-show="showImportRiskModal" 
         x-data="{ 
            isUploading: false,
            fileName: '',
            handleFile(e) {
        ...
        <!-- Backdrop -->
        <div @click="!isUploading && (showImportRiskModal = false)" class="fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-sm"
        ...
        <div @click.stop class="relative w-full rounded-3xl bg-white dark:bg-gray-900 max-w-[700px] shadow-2xl border border-gray-100 dark:border-gray-800"
            ...
            <div class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
                ...
                <div class="px-2 pr-14 mb-6">
                    <h4 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Import Tunjangan Risiko PHL</h4>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 font-medium">Unggah file Excel untuk memasukkan data tunjangan risiko periode ini.</p>
                </div>

                <!-- Form Content -->
                <form id="importRiskPhlForm" action="{{ $period ? url('/payroll/phl/periods/' . $period->id . '/import-risk') : '#' }}" method="POST" enctype="multipart/form-data" @submit="isUploading = true" class="flex flex-col">
                    @csrf
                    ...
                    <div class="custom-scrollbar max-h-[458px] overflow-y-auto p-2 space-y-6">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400 leading-relaxed font-medium">
                                        Pastikan file Anda memiliki kolom-kolom berikut: <span class="font-bold text-gray-700 dark:text-gray-300">ID Karyawan, Nama Karyawan, Tanggal (DD-MM-YYYY), Nominal, Keterangan</span>.
                                        <br><span class="text-brand-600 dark:text-brand-400 font-bold">*Catatan:</span> Kolom <span class="font-bold">Nominal</span> dapat dikosongkan jika Anda ingin menggunakan nominal bawaan dari data karyawan.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="rounded-2xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-transparent flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Unduh template tunjangan risiko PHL yang sesuai standar.</p>
                                </div>
                            </div>
                            <a href="/templates/risk_import_template_phl.xlsx" download class="inline-flex items-center justify-center font-medium gap-2 rounded-xl transition bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03] dark:hover:text-gray-300 px-4 py-2.5 text-xs font-semibold">
                                Unduh Template
                            </a>
                        </div>
                        ...
                        <div class="relative group">
                            <input type="file" name="file" accept=".xlsx, .xls, .csv" class="hidden" id="riskPhlFileInput" @change="handleFile($event)" required>
                            <label for="riskPhlFileInput" class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-300 rounded-2xl cursor-pointer bg-gray-50 hover:bg-gray-100 dark:border-gray-700 dark:bg-dark-900 dark:hover:bg-gray-800 transition-all group-hover:border-brand-500">
