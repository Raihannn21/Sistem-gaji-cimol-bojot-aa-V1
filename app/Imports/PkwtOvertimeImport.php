...
class PkwtOvertimeImport implements ToModel, WithHeadingRow
{
    protected $periodId;
    ...
    public function model(array $row)
    {
        ...
        if (!$employee) {
            Log::warning("Employee with ID {$noId} not found or not PKWT during overtime import.");
            $this->skippedCount++;
        ...
        try {
            ...
            $existing = PkwtOvertime::where('pkwt_payroll_period_id', $this->periodId)
            ...
            Log::error("Error importing PKWT overtime row: " . $e->getMessage());
            $this->skippedCount++;
            return null;
        }
    }
    ...
    private function transformDate($value)
    {
        ...
        try {
            ...
            Log::error("Date parsing error in PKWT overtime import for value '{$value}': " . $e->getMessage());
            return null;
        }
    }
